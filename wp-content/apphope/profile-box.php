	...
	<div class="row">
		...
		<div class="col-lg-9 main">
			<div class="row">
				...
				<div class="col-lg-6 desktop">Newest Dream On Platform</div>
			</div>
		</div>
	</div>
	</h5>
		...
		foreach ($id_orders as $id_order) {
			...
			if(!in_array($id_order->product_id, $listed)){
				$listed[] = $id_order->product_id;$wp_ids[] = $project->get_project_postid();
					...
					<?php $user_ID = get_current_user_id(); $wp_ids = array(); $args = array('post_type' => 'ignition_product',/*'author' => $user_id,*/ 'post__in' =>$wp_ids, 'post_status' => array('publish', 'closed'), 'posts_per_page' => 1, 'order'=>'DESC','orderby'=>'date' );
								$the_query = new WP_Query( $args );
					?>
					<?php if ($the_query->have_posts()) : $i=0;while ($the_query-> have_posts() ) : $the_query->the_post();$i++;?>
						<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'grid-thumb' ); ?>
