	...
	function idc_lightbox_price_change(price) {
		...
		console.log(price);
		if(price){
			var price_new = parseFloat(levelPrice) * (2 / 100);
			price = price_new + parseFloat(levelPrice);
		}
		if (price < levelPrice) {
...
jQuery(document).ready(function() {
	/*jQuery('.ignitiondeckchck').magnificPopup({
		type: 'inline'
	});*/
	
	jQuery('body').on('change','.idc_lightbox:visible input[name="total"]',function() {
		var price = jQuery(this).val();
		var cleanPrice = price.replace(/[^0-9\.]+/g, '');
		// console.log(cleanPrice);
		idc_lightbox_price_change(cleanPrice);
	});
	jQuery('body').on('change','.idc_lightbox:visible select[name="level_select"]',function() {
		var levelPrice = jQuery('.idc_lightbox:visible select[name="level_select"] :selected').data('price');
		idc_lightbox_price_change(levelPrice);
	});
	...
	jQuery('body').on('click','input[name="lb_level_submit"]',function(e) {
		e.preventDefault();
		...
		if (!error) {
			//jQuery( ".mfp-content" ).html('<div class="ignitiondeck idc_lightbox"><div class="lb_wrapper"></div></div>')
			var content = jQuery( ".mfp-content .ignitiondeck" ).html();
			jQuery( "#ignitiondeckchck" ).html(content);
			jQuery( ".mfp-content .ignitiondeck" ).load(formAction);
			
			//jQuery( "#ignitiondeckchck" ).load(formAction,function() {jQuery('.ignitiondeckchck').trigger('click');});
			//console.log('434');
			//jQuery('.idc_lightbox:visible').magnificPopup.close();
			
			//jQuery.magnificPopup.close();
			//openLBGlobal(jQuery( ".ignitiondeck.chck" ), null,null);
			//nikita jQuery('.idc_lightbox:visible form').submit();
		}
	});
});
	jQuery('.main-submit-wrapper #reset').on('click',function(){
		/*var magnificPopupi = jQuery.magnificPopup.instance; 
		// save instance in magnificPopup variable
		console.log('333');
		magnificPopupi.close();jQuery.magnificPopup.close(); */
		var content = jQuery( "#ignitiondeckchck" ).html();
		jQuery( ".mfp-content .ignitiondeck" ).html(content);
	...
	jQuery.magnificPopup.open({
		...
		callbacks: {
			open: function() {
				...
				var readeronly = jQuery('.idc_lightbox:visible select[name="level_select"] :selected').text();
				if(readeronly == 'No Incentive, just sponsoring') {
					jQuery('#total').removeAttr('readonly');
				} else {
					jQuery('#total').attr('readonly', true);
