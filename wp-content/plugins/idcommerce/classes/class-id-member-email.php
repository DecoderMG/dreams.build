...
class ID_Member_Email {
	...
	function send_mail() {
		...
		*/
		...
		if (empty($coname)) $coname = '';
		if (empty($coemail)) $coemail = get_option('admin_email', null);
		...
		*/
		...
		if (!empty($coemail)) {
			...
			if (isset($enable_sendgrid) && $enable_sendgrid == 1) {
				...
				//return $this->to."<br>".$this->subject."<br>".$this->message;
				if (empty($headers)) {
					...
					$headers = array(
						__('From', 'memberdeck').': '.$coname.' <'.$coemail.'>',
						__('Reply-To', 'memberdeck').': '.$coemail,
						"MIME-Version: 1.0",
						"Content-Type: text/html; charset=UTF-8",
					);
				}
				$go = wp_mail($this->to, $this->subject, $this->message, $headers);
