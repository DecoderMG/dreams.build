...
class ID_Member {
	...
	public static function export_members($product_id, $is_project = false) {
		...
		$level_array = array();
		$product_ids = array();
		$post_id = (int)$_GET['export_project'];
		...
		if ($is_project) {
			...
			foreach ($mdid_assignments as $assignment) {
				...
				$level_array[$product_id] = ID_Member_Level::get_level($product_id);
		...
		// Get project summary
		$summary = the_project_summary($post_id);

		$stage = array();

		// Stages end time
		$project_stage_1_timestamp = DateTime::CreateFromFormat('m/d/Y', $summary->end1);
		$project_stage_1_timestamp = strtotime($project_stage_1_timestamp->format('Y-m-d'));
		$project_stage_2_timestamp = DateTime::CreateFromFormat('m/d/Y', $summary->end2);
		$project_stage_2_timestamp = strtotime($project_stage_2_timestamp->format('Y-m-d'));


		if ($project_stage_1_timestamp >= time()) {
			$stage = array(1);
			// $product_ids = array(0 => reset($available_products));
		}

		if ($project_stage_2_timestamp > time()) {
			$stage = array(1);
			// $product_ids = array(0 => reset($available_products));
		}

		if ((time() > $project_stage_1_timestamp && time() > $project_stage_2_timestamp)) {
			$stage = array(1,2);
		...
		foreach($product_ids as $product_id) {
			$members = self::get_allowed_users_new($product_id);
			...
			if (!empty($members)) {
				foreach ($members as $member) {
					...
					$order_stage = $member->stage;
					// Lets locate stage by order and stage date
//					if (strtotime($project_stage_1_timestamp) > strtotime($member->order_date)) {
//						$order_stage = 1;
//					} else {
//						$order_stage = 2;
//					}

                    if(empty($member->stage)){
                        $order_stage = $member->stage;
                    }
					...
					foreach ($member as $k=>$v) {
						if (is_string($v)) {
							if ($k == 'level_id') {
								...
								$user['project'] = get_the_title($post_id);//$level_array[$v]->level_name;
								$user['product_name'] = $level_array[$v]->level_name;
								$user['incentive_price'] = $level_array[$v]->level_price;
							}
							else {
								$user[$k] = $v;
							}
					...
					// Skip the row if stage not in list
					if (in_array($order_stage, $stage)) {
						$user_records[] = $user;
					}
				}
			}
			...
			if (!empty($user_records)) {
				// $user_records[] = array('Message' => 'no data');

			// now we should have data to export
			$filename = __('IDC Customer Export', 'memberdeck').'-'.date('Y-m-d h-i-s').' (pl'.$product_id.')';
		...
		if ($filenames_count > 0) {
			...
			if ($filenames_count > 1) {
				...
				header('Content-Disposition: attachment; filename='.rawurlencode($filenames[0]).'.csv');
				header('Pragma: no-cache');
