...
//        die(print_r($_POST));
		...
		if (isset($_GET['edit-profile']) && $_GET['edit-profile'] == $user_id /*&& $segments[0]=='dashboard'*/) {
			if (isset($_POST['edit-profile-submit'])) {
				$user_firstname = esc_attr($_POST['first-name']);
				$user_lastname = esc_attr($_POST['last-name']);
				$description = esc_attr($_POST['description']);
				$full_description = esc_attr($_POST['full_description']);
				...
				$city = esc_attr($_POST['user_city']);
				$state = esc_attr($_POST['user_state']);
				$country = esc_attr($_POST['user_country']);
				$zipcode = esc_attr($_POST['user_zipcode']);
				$user_map_lng = esc_attr($_POST['user_map_lng']);
				$user_map_lat = esc_attr($_POST['user_map_lat']);
				$up_array = array(
						'ID' => $user_id,
				...
				/*$user_addr = array(
						'user_map_lng' => $user_map_lng,
						'user_map_lat' => $user_map_lat,
						'zipcode' => $zipcode,
						'city' => $city,
						'state' => $state,
						'country' => $country
				);
				update_user_meta($user_id, 'user_addr', $user_addr);*/
				$user_coord = array(
						'user_map_lng' => $user_map_lng,
						'user_map_lat' => $user_map_lat
				);
				update_user_meta($user_id, 'user_coord', $user_coord);
				...
				update_user_meta($user_id, 'full_description', $full_description);
...
function memberdeck_account_check() {
	if (is_user_logged_in()) {
		global $current_user;
		get_currentuserinfo();
		$user_id = $current_user->ID;
		...
		if (isset($_GET['edit-account']) && $_GET['edit-account'] == $user_id /*&& $segments[0]=='dashboard'*/) {

            if (count($_POST) > 0) {

				$email = esc_attr($_POST['email']);
				$cemail = esc_attr($_POST['cemail']);
				...
				if ($pw == $cpw && !empty($pw)){
                    $up_array['user_pass'] = $pw;
                    // Redefining user_login ensures we return the right case in the email.
                    $user_login = $nicename;
                    $user_email = $email;

                    $message = get_option('password_changed');
                    ...
                    $title = sprintf( __('[%s] Password Was Reset'), $blogname );

                    if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) )
                        wp_die( __('The email could not be sent.') . "<br />\n" . __('Possible reason: your host may have disabled the mail() function.') );

				}
				if ($email == $cemail && !empty($email )) $up_array['user_email'] = $email;
				wp_update_user($up_array);
				update_user_meta($user_id, 'instant_checkout', $instant_checkout);
			}
			add_filter('the_content', 'memberdeck_account_form');
		}
		else if (isset($_GET['edit-account'])) {
			echo '<script>location.href="?edit-account='.$user_id.'";</script>';
		}
	}
}

add_action('init', 'memberdeck_account_check');
...
function md_shipping_info() {
	...
	$coord_info = get_user_meta($user_id, 'user_coord', true);

	if(isset($coord_info['user_map_lat']) && isset($coord_info['user_map_lng'])) {
		$user_map_lat = $coord_info['user_map_lat'];
		$user_map_lng = $coord_info['user_map_lng'];
	}
	...
	if (isset($_POST['edit-profile-submit'])) {
		...
		update_user_meta($user_id, 'user_country', $country);
		update_user_meta($user_id, 'user_state', $state);
		update_user_meta($user_id, 'zipcode', esc_attr($_POST['zip']));
	}
...
function idc_custom_email($user_id, $email, $props) {

    $umeta = get_user_meta( $user_id);

    if(isset($umeta[$props['template']]) && $umeta[$props['template']][0] == 0 ){
        return false;
    }
	...
	//todo - collapse into md5 on production
	$is_sent = get_user_meta( $user_id, 'sent_'.$props['post_id'].'_'.$props['template'].'_'.$prop_id, true);
	if ($is_sent) {
		return false;
	}

	// Let's flag this event as sent
	update_user_meta( $user_id, 'sent_'.$props['post_id'].'_'.$props['template'].'_'.$prop_id, '1');
	...
	if ($props['post_id']) {
		...
		$project_link = get_the_permalink($post->ID);
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'grid-thumb' );
		if (!empty($post)) {
	...
	$site_name = get_bloginfo('name');
	$subject = $site_name.' '. ($props['subject'] ? $props['subject'] : __('Notification', 'memberdeck'));
	$durl = md_get_durl();
	$message = '<html><body width="100%" bgcolor="#fff" style="margin: 0 auto !important;padding: 0 !important;height: 100% !important;width: 100% !important;mso-line-height-rule: exactly;">';
	// $text = get_option('welcome_email');
	if (empty($text)) {
		$text = get_option($props['template']);
	}
	if (empty($text)) {
		$message .= '<h4>Error occurred during processing email template.</h4><p>Template {{'.$props['template'].'}} is empty.</p>';
	}
	else {
		$merge_swap[] = array(
				'tag' => '{{END_DATE}}',
...
//                echo $txn_type;
            ...
            if ((isset($success) && $success == true) || (isset($preauth) && $preauth == true)) {
                ...
                if (isset($newuser)) {
                    ...
                    $user_id = wp_insert_user(array('user_email' => $email,
                        'user_login' => $email,
                        'user_pass' => $pw,
                        'first_name' => $fname,
                        'last_name' => $lname,
                    ...
                    if (!empty($user_id)) {
                        ...
                        update_user_meta($user_id, 'user_zipcode', $customer['zip']);
                        update_user_meta($user_id, 'user_city', $customer['city']);
                        update_user_meta($user_id, 'user_state', $customer['state']);
                        update_user_meta($user_id, 'user_country', $customer['country']);
                    ...
                    if (isset($_POST['Fields'])) {
                        ...
                        if (isset($project_id) && isset($proj_level)) {
                            ...
                            if (isset($new_order)) {
                                ...
                                $order->update_order_stage($stage);
                            ...
                            $level_check = ID_Member_Level::get_level($proj_level);


							$project_currency = get_post_meta($post_id, 'ign_currency', true);
                            if( $level_check->level_price == 0 ){
                                $pay_id = mdid_insert_payinfo($fname, $lname, $email, $project_id, $txn_id, $proj_level, currencyNew(getSymbolCode(), $project_currency, $price, true), $status, $created_at, $newcharge->card->last4);
                             } else {
                                $per = round($level_check->level_price * (2 / 100), 2);
                                if(($level_check->level_price + $per) == $price){
									if(getSymbolCode() == $project_currency) {
										$pay_id = mdid_insert_payinfo($fname, $lname, $email, $project_id, $txn_id, $proj_level, $price, $status, $created_at, $newcharge->card->last4);
									} else {
										$pay_id = mdid_insert_payinfo($fname, $lname, $email, $project_id, $txn_id, $proj_level, currencyNew(getSymbolCode(), $project_currency, $price, true), $status, $created_at, $newcharge->card->last4);
									}
								} else {
...
//                            $pay_id = mdid_insert_payinfo($fname, $lname, $email, $project_id, $txn_id, $proj_level, currencyNew(getSymbolCode(), 'USD', $price, true), $status, $created_at, $newcharge->card->last4);
...
//                                $percent = ID_Project::set_status_for_all_project();
//                                echo '<pre>';
////                                print_r($type);
//                                print_r($newcharge->application_fee);
//                                print_r(Stripe_ApplicationFee::retrieve('fee_9S8w60f2UNr2ND'));
//                                echo '</pre>';
//die();// 27322 - 27326 5 ore
                                if(isset($newcharge)){
                                    if($newcharge->application_fee != ''){
                                        $aplication_fee = Stripe_ApplicationFee::retrieve($newcharge->application_fee);
                                        $aplication_fee->refund();
                                    }
                                    $newcharge->refund();

                                    do_action('idc_send_notifications_project', $project_id);
                                }
                            }
                        }
                    }
                }
                if (is_user_logged_in() && isset($customer['address'])) {
                    global $current_user;
                    get_currentuserinfo();
                    $user_id = $current_user->ID;
                    update_user_meta($user_id, 'user_zipcode', $customer['zip']);
					...
					if (!empty($level)) {
						...
						$content .= '<li class="backer_list_item backers_tab content_tab">
										...
										<div class="backer_list_namedate">
											<div class="backer_list_name">'.(is_id_pro() && isset($user_info->ID) ? '<a href="'.get_site_url().'/author/'.$user_info->user_login.'">' : '').apply_filters('idc_backers_listing_name', (isset($user_info->display_name) ? $user_info->display_name : __('Anonymous', 'memberdeck')), $idc_order->id, $idc_order->user_id).(is_id_pro() && isset($user_info->ID) ? '</a>' : '').'</div>
...
function mdid_insert_payinfo($fname = null, $lname = null, $email = null, $project_id, $transaction_id, $proj_level, $price, $status = 'P', $created_at = null, $stripe_card_id) {
	...
	$sql = $wpdb->prepare('INSERT INTO '.$wpdb->prefix.'ign_pay_info (first_name,
					...
					%s,
					%s
					)', $fname, $lname, $email, $project_id, $transaction_id, $proj_level, $price, $status, $created_at, $stripe_card_id);
	$res = $wpdb->query($sql);
