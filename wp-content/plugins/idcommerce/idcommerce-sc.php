...
function md_sc_account($user_id) {
    global $wpdb;
    global $stripe_api_version;
    $params = get_sc_params($user_id);
    if (isset($params->id) && $params->id > 0) {
...
add_action('init', 'md_sc_remove');

function md_sc_remove() {
	if (isset($_GET['sc_delete']) && $_GET['sc_delete'] == 'yes') {
			global $current_user;
			get_currentuserinfo();
			$user_id = $current_user->ID;
			delete_sc_params($user_id);
            wp_redirect( $_SERVER['SERVER_NAME'].'/dashboard/?edit-account='.$user_id.'&stripe_disconnect=yes' );
            die;
	}
}
									...
									if (isset($response->error_description)) {
										...
										if ($insert_id > 0) {
                                            ...
                                            if($stripe_account->details_submitted == true){
                                                $stripe_message = 'Your stripe connected';
//                                                die($stripe_message);
                                            }
