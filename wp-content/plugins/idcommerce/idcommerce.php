...
function idc_init() {
	...
	idc_set_roles();
//    add_action('idc_hourly_event', 'idc_send_notifications');
...
function idc_set_defaults() {
	...
	if (function_exists('is_id_pro') && is_id_pro()) {
		$success_notification = 
			...
			Congratulations! Thanks to the help of sponsors like you, {{PROJECT_NAME}} has successfully reached its funding goal.
		...
		$remind_me_later = 
			'<h3>{{PROJECT_NAME}} Funding Ends soon!</h3>
			Warning! {{PROJECT_NAME}} has almost reached its funding end date ({{END_DATE}}).
	...
	if (function_exists('is_id_pro') && is_id_pro()) {
		update_option('remind_me_later_default', $remind_me_later);
...
function idc_hourly_scheduler() {
	$args = array();
	wp_schedule_event(time(), 'hourly', 'idc_hourly_event', $args);
}
...
add_action('idc_hourly_event', 'idc_send_notifications');
function idc_send_notifications() {
	// Getting orders only that are of standard level
	// Looping forever and breaking loop when orders are done, picking order in chunks of 200
//	$projects = ID_Project::get_all_projects();
//
//    ID_Project::writeCronLog('======== Notific =================== New cron hourly run =================== data '.date('Y-m-d h:s'));
//    ID_Project::writeCronLog('=== projects = '.count($projects));
//	$actions = array();
//
...
//        if( $status_project == 'pending' ) continue;
//
//
//        $actions = array();
//
//		$raised = $a_project->get_project_raised();
//		$goal = get_post_meta($post_id, 'ign_fund_goal', true);
//		$contact_sponsors = false;
//
//		$project_id = get_post_meta($post_id, 'ign_project_id', true);
//
//		if ($raised >= ($goal*0.5) && empty(get_post_meta($post_id, 'notified_funding_reaches_50', true))) {
//			// update_post_meta($post_id, 'notified_funding_reaches_50', 1);
//			$actions['funding_reaches_50'] = 'funding_reaches_50';
//			$contact_sponsors = true;
//		}
//
//		if ($raised >= ($goal*0.75) && empty(get_post_meta($post_id, 'notified_funding_reaches_75', true))) {
//			// update_post_meta($post_id, 'notified_funding_reaches_75', 1);
//			$actions['funding_reaches_75'] = 'funding_reaches_75';
//			if (isset($actions['funding_reaches_50'])) {
//				unset($actions['funding_reaches_50']);
//			}
//			$contact_sponsors = true;
//		}
//
//        ID_Project::writeCronLog('= '.$raised.'= '. $goal .' = Notific project: '.$project->id.'  = '.implode(', `', $actions));
//
//
//		$sponsors = array();
//
//		if ($contact_sponsors == true) {
//			global $wpdb;
//			global $crowdfunding;
//			$level_array = array();
//			$product_ids = array();
...
//				$members = ID_Member::get_allowed_users_new($product_id);
//				foreach($members as $member) {
//					$user_data = get_userdata($member->user_id);
//					if (!isset($sponsors[$member->user_id])) {
//						$sponsors[$member->user_id] = $user_data;
//					}
//				}
//			}
//			foreach($sponsors as $sponsor) {
//				foreach($actions as $action) {
//					idc_custom_email($sponsor->id, $sponsor->data->user_email, $props = array(
//						'template' => $action,
//						'subject' => $action,
//						'post_id' => $project->id,
//						'user' => $sponsor
//					));
//
//                    ID_Project::writeCronLog('=== Notific project: '.$project->id.' ; Sponsor: '.$sponsor->data->user_email.'  = '.implode(', `', $actions));
//				}
//			}
//		}
//	}
//	if (count($actions) > 0) {
//		ID_Project::writeCronLog('=== actions performed = '.implode(', `', $actions));
//	}

}

add_action('idc_send_notifications_project', 'idc_send_notifications_project');
function idc_send_notifications_project($project_id) {
	// Getting orders only that are of standard level
	// Looping forever and breaking loop when orders are done, picking order in chunks of 200
//	$projects = ID_Project::get_all_projects();

    ID_Project::writeCronLog('======== Notification =================== New notification project =================== data '.date('Y-m-d h:s'));

    $a_project = new ID_Project($project_id);
    $post_id = $a_project->get_project_postid();
    ...
    $raised = $a_project->get_project_raised();
    $goal = $a_project->the_goal();
    $contact_sponsors = false;
//
//    $project_id = get_post_meta($project_id, 'ign_project_id', true);

    if ($raised >= ($goal*0.5) && empty(get_post_meta($project_id, 'notified_funding_reaches_50', true))) {
        // update_post_meta($project->id, 'notified_funding_reaches_50', 1);
        $actions['funding_reaches_50'] = 'funding_reaches_50';
//        $contact_sponsors = true;
    }

    if ($raised >= ($goal*0.75) && empty(get_post_meta($project_id, 'notified_funding_reaches_75', true))) {
        // update_post_meta($project->id, 'notified_funding_reaches_75', 1);
        $actions['funding_reaches_75'] = 'funding_reaches_75';
        if (isset($actions['funding_reaches_50'])) {
            unset($actions['funding_reaches_50']);
        }
//        $contact_sponsors = true;
    ...
    foreach ($actions as $action){
        ...
        ID_Project::writeCronLog('======== Notific ='.$action.'== send: '.$user_data->user_email);
    }
...
add_action('idc_send_notifications_sponsors', 'idc_send_notifications_sponsors');
function idc_send_notifications_sponsors($agr) {
    ...
    if (!empty($project_orders)) {
        ...
        if (!empty($mdid_orders)) {
            foreach ($mdid_orders as $mdid_order) {
                ...
                if (!empty($idc_order)) {
                    if(!isset($sponsors[$idc_order->user_id])){
                        $user_data = get_userdata($idc_order->user_id);
                        $sponsors[$idc_order->user_id] = $user_data;
                    }
    ...
    foreach($sponsors as $sponsor){
        idc_custom_email($sponsor->ID, $sponsor->user_email, $props = array(
            'template' => $action,
            'subject' => $subject,
            'post_id' => $post_id,
            'user' => $sponsor
        ));
        ID_Project::writeCronLog('Sponsors send email:'. $sponsor->user_email);
    }
}

add_action( 'comment_post', 'send_notification_message_function', 10, 2 );
function send_notification_message_function( $comment_ID, $comment_approved ) {
    if( 1 === $comment_approved ){
        ...
        $project_id = get_post_meta($post_id, 'ign_project_id', true);
        do_action('idc_send_notifications_sponsors', array($project_id, 'project_sponsor_innovator_comment', 'New Comment Added'));
    }
}
...
function memberdeck_disable_others() {
	$get_array = array('payment_settings', 'backer_profile', 'edit-profile','friends', 'creator_projects', 'mdid_checkout', 'idc_orders', 'key_valid');
