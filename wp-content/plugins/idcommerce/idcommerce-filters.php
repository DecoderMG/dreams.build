...
function memberdeck_profile_form($content) {
	...
	if (isset($usermeta['full_description'][0])) $full_description = $usermeta['full_description'][0];
	if (isset($usermeta['twitter'][0])) $twitter = $usermeta['twitter'][0];
	if (isset($usermeta['facebook'])) $facebook = $usermeta['facebook'][0];
	if (isset($usermeta['google'])) $google = $usermeta['google'][0];
	if (isset($usermeta['instagram'])) $instagram = $usermeta['instagram'][0];
	if (isset($usermeta['linkedin'])) $linkedin = $usermeta['linkedin'][0];

    $usermeta['user_coord'] = unserialize($usermeta['user_coord'][0]);
    $usermeta['md_shipping_info'] = unserialize($usermeta['md_shipping_info'][0]);
    $usermeta['user_addr'] = unserialize($usermeta['user_addr'][0]);

    if(isset($usermeta['user_coord'])){
		if (isset($usermeta['user_coord']['user_map_lng'])) $user_map_lng = $usermeta['user_coord']['user_map_lng'];
		if (isset($usermeta['user_coord']['user_map_lat'])) $user_map_lat = $usermeta['user_coord']['user_map_lat'];
	}
		if (isset($usermeta['user_city'])) $city = $usermeta['user_city'][0];
		if (isset($usermeta['user_country'])) $country = $usermeta['user_country'][0];
		if (isset($usermeta['user_state'])) $state = $usermeta['user_state'][0];
		if (isset($usermeta['user_zipcode'])) $zipcode = $usermeta['user_zipcode'][0];
	...
	if (isset($_POST['edit-profile-submit'])) {
		...
		$user_country = esc_attr($_POST['country']);
		$user_zipcode = esc_attr($_POST['zipcode']);
...
/**
 * Filter called from memberdeck_account_check() in idc-functions
 */
function memberdeck_account_form($content) {
	ob_start();
	global $current_user;
	global $first_data;
	global $stripe_api_version;
	...
	if (isset($_POST['edit-profile-submit'])) {
		...
		if (isset($_POST['pw'])) $pw = esc_attr($_POST['pw']);
		if (isset($_POST['cpw']))  $cpw = esc_attr($_POST['cpw']);
		$description = esc_attr($_POST['description']);
	...
	if (!empty($pw) && strlen($pw)<8) {
		$error .= __('<br/>Passwords should be at least eight (8) characters long', 'memberdeck');
	}
	if (!empty($pw) && $pw !== $cpw) {
		$error .= __('<br/>Passwords do not match', 'memberdeck');
	}
	if (!empty($cemail) && $email !== $cemail) {
		$error .= __('<br/>Emails do not match', 'memberdeck');
	}
	if (isset($_GET['edited'])&&empty($error)) {
		unset($_POST);
		$success = __('Account Updated!', 'memberdeck');
	}

	include 'templates/_editAccount.php';
	$content = ob_get_contents();
	ob_end_clean();
	return $content;
}
...
function idc_display_checkout_descriptions($content, $level, $level_price, $user_data, $settings, $general, $currency) {
	...
	$cc_currency_symbol = md_currency_symbol(strtoupper($currency));
	$cc_currency = $currency;
	// Stripe currency
	$stripe_currency = $currency;
	$stripe_symbol = md_currency_symbol(strtoupper($currency));
	if (!empty($settings)) {
		if (is_array($settings)) {
