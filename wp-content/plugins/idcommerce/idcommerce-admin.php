...
add_action( 'user_register', 'add_notification_for_new_user', 10, 1 );

function add_notification_for_new_user( $user_id ) {
    $fields = array(
...
function idc_email() {
	...
	else if (isset($_POST['restore_default_remind_me_later'])) {
		$remind_me_later = get_option('remind_me_later_default');
		update_option('remind_me_later', $remind_me_later);
	}
	...
	else if (isset($_POST['restore_default_reset_password'])) {
		$welcome_email = get_option('reset_password_default');
		update_option('reset_password', $reset_password);
	}

	else if (isset($_POST['restore_default_password_changed'])) {
		$welcome_email = get_option('password_changed_default');
		update_option('password_changed', $password_changed);
	}

	else if (isset($_POST['restore_default_account_deleted'])) {
		$welcome_email = get_option('account_deleted_default');
		update_option('account_deleted', $account_deleted);
	}

	else if (isset($_POST['restore_default_product_renewal_email'])) {
		$welcome_email = get_option('product_renewal_email_default');
		update_option('product_renewal_email', $product_renewal_email);
	}

	else if (isset($_POST['restore_default_project_approved'])) {
		$welcome_email = get_option('project_approved_default');
		update_option('project_approved', $project_approved);
	}

	else if (isset($_POST['restore_default_project_live'])) {
		$welcome_email = get_option('project_live_default');
		update_option('project_live', $project_live);
	}

	else if (isset($_POST['restore_default_project_rejected'])) {
		$welcome_email = get_option('project_rejected_default');
		update_option('project_rejected', $project_rejected);
	}

	else if (isset($_POST['restore_default_project_funded'])) {
		$welcome_email = get_option('project_funded_default');
		update_option('project_funded', $project_funded);
	}

	else if (isset($_POST['restore_default_project_not_funded'])) {
		$welcome_email = get_option('project_not_funded_default');
		update_option('project_not_funded', $project_not_funded);
	}

	else if (isset($_POST['restore_default_project_stage_change'])) {
		$welcome_email = get_option('project_stage_change_default');
		update_option('project_stage_change', $project_stage_change);
	}

	else if (isset($_POST['restore_default_project_complete_stage2'])) {
		$welcome_email = get_option('project_complete_stage2_default');
		update_option('project_complete_stage2', $project_complete_stage2);
	}

	else if (isset($_POST['restore_default_project_new_comment'])) {
		$welcome_email = get_option('project_new_comment_default');
		update_option('project_new_comment', $project_new_comment);
	}

	else if (isset($_POST['restore_default_project_sponsor_new_update'])) {
		$welcome_email = get_option('project_sponsor_new_update_default');
		update_option('project_sponsor_new_update', $project_sponsor_new_update);
	}

	else if (isset($_POST['restore_default_project_sponsor_failed'])) {
		$welcome_email = get_option('project_sponsor_failed_default');
		update_option('project_sponsor_failed', $project_sponsor_failed);
	}

	else if (isset($_POST['restore_default_project_sponsor_stage_change'])) {
		$welcome_email = get_option('project_sponsor_stage_change_default');
		update_option('project_sponsor_stage_change', $project_sponsor_stage_change);
	}

	else if (isset($_POST['restore_default_project_sponsor_complete_stage2'])) {
		$welcome_email = get_option('project_sponsor_complete_stage2_default');
		update_option('project_sponsor_complete_stage2', $project_sponsor_complete_stage2);
	}

	else if (isset($_POST['restore_default_project_sponsor_innovator_comment'])) {
		$welcome_email = get_option('project_sponsor_innovator_comment_default');
		update_option('project_sponsor_innovator_comment', $project_sponsor_innovator_comment);
	}

	else if (isset($_POST['restore_default_project_sponsor_innovator_update'])) {
		$welcome_email = get_option('project_sponsor_innovator_update_default');
		update_option('project_sponsor_innovator_update', $project_sponsor_innovator_update);
	}

	else if (isset($_POST['restore_default_funding_reaches_50'])) {
		$welcome_email = get_option('funding_reaches_50_default');
		update_option('funding_reaches_50', $funding_reaches_50);
	}

	else if (isset($_POST['restore_default_funding_reaches_75'])) {
		$welcome_email = get_option('funding_reaches_75_default');
		update_option('funding_reaches_75', $funding_reaches_75);
	}
