...
function memberdeck_dashboard() {
	...
	if (isset($usermeta['city'])) $city = $usermeta['city'][0];
	if (isset($usermeta['country'])) $country = $usermeta['country'][0];
	if (isset($usermeta['state'])) $state = $usermeta['state'][0];
	if (isset($usermeta['zipcode'])) $zipcode = $usermeta['zipcode'][0];
	
		include_once 'templates/_editProfile.php';
...
//	}
	...
	if ($crowdfunding) {
		...
		if (!empty($cf_assignments)) {
			...
			$stage = ($total_ >=$goal_)?'2':'1';
		}
	}
	/*godric*/
	//$txn_type = ($stage==='2')?'capture':$return->txn_type;
