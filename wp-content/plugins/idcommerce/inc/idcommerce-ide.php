		...
		if (md_ide_opt_in_required()){
			if ($enable_creator || !empty($user_projects)) {
				if (idc_creator_settings_enabled()) {
					/*nikita echo '<li '.(isset($_GET['payment_settings']) ? 'class="active"' : '').'><a href="'.md_get_durl().$prefix.'payment_settings=1">'.__('Payment Account', 'memberdeck').'</a></li>';*/
				}
				echo '<div class="col-lg-2 '.(isset($_GET['creator_projects']) || isset($_GET['create_project']) || isset($_GET['project_files']) ? 'active' : '').'"><a href="'.$url.'">'.$tab.'</a></div>';
			}
		} else {
			if (idc_creator_settings_enabled()) {
				/*nikita echo '<li '.(isset($_GET['payment_settings']) ? 'class="active"' : '').'><a href="'.md_get_durl().$prefix.'payment_settings=1">'.__('Payment Account', 'memberdeck').'</a></li>';*/
...
function md_ide_check_creator_profile() {
	...
	if (isset($_GET['friends']) && $_GET['friends'] == 1) {
		add_filter('the_content', 'md_ide_friends');
	}
	if (isset($_GET['notifications']) && $_GET['notifications'] == 1) {
		add_filter('the_content', 'md_ide_notifications');
	}
	if (isset($_GET['analytics'])) {
		add_filter('the_content', 'md_ide_analytics');
	}
}
function md_ide_analytics($content) {
	ob_start();
	global $current_user;global $permalink_structure;
	if (empty($permalink_structure)) {$prefix = '&';} else {$prefix = '?';}
	get_currentuserinfo();
	$user_id = $current_user->ID;
	if (!is_user_logged_in() ) {$durl = md_get_durl(); echo '<script>window.location="'.$durl.'";</script>';}
	echo '<div class="memberdeck">';
	include_once IDC_PATH.'templates/_mdProfileTabs.php';
	echo '<div class="col-lg-12"><div class="whitebox">';
	//include get_stylesheet_directory_uri().'/inc/_myFriends.php';
	include get_template_directory().'/../apphope/inc/_myAnalytics.php';
	echo '</div></div></div>';
	echo '</div>';
...
function md_ide_friends($content) {
	ob_start();
	global $current_user;global $permalink_structure;
	if (empty($permalink_structure)) {$prefix = '&';} else {$prefix = '?';}
	get_currentuserinfo();
	...
	echo '<div class="memberdeck">';
	include_once IDC_PATH.'templates/_mdProfileTabs.php';
	echo '<div class="row mt"><div class="col-lg-2 dashboard-sidebar">
			<h4>Friends Projects</h4>
			<div class="whitebox parted">
				<a href="#created" class="scroll title">CREATED BY</a>
				<a href="#backed" class="scroll title">BACKED BY</a>
			</div>
		</div><div class="col-lg-10"><div class="whitebox"><div class="dashboard-block nb">';
	//include get_stylesheet_directory_uri().'/inc/_myFriends.php';
	include get_template_directory().'/../apphope/inc/_myFriends.php';
...
function md_ide_notifications($content) {
	ob_start();
	global $current_user;global $permalink_structure;
	if (empty($permalink_structure)) {$prefix = '&';} else {$prefix = '?';}
	get_currentuserinfo();
	...
	if($_POST['submit']=='Update Settings'){
		$notify_create_project = intval($_POST['cancel_notify_create_project'])?0:1;
		$notify_update_project = intval($_POST['cancel_notify_update_project'])?0:1;
		$notify_publish_project = intval($_POST['cancel_notify_publish_project'])?0:1;
		update_user_meta($user_id, 'cancel_notify_create_project', $notify_create_project);
		update_user_meta($user_id, 'cancel_notify_update_project', $notify_update_project);
		update_user_meta($user_id, 'cancel_notify_publish_project', $notify_publish_project);
	};
	echo '<div class="memberdeck">';
	include_once IDC_PATH.'templates/_mdProfileTabs.php';
	//print_r($_POST);
	//include get_stylesheet_directory_uri().'/inc/_myFriends.php';
	include get_template_directory().'/../apphope/inc/_myNotifications.php';
	echo '</div>';
	$content = ob_get_contents();
	ob_end_clean();
	return $content;
...
function md_ide_creator_projects($content) {
	...
	echo '<div class="row mt"><div class="col-lg-2 dashboard-sidebar">
			<div class="whitebox parted">
				<a href="#your_dreams" class="scroll title">YOUR DREAMS</a>
				<a href="#sponsored" class="scroll title">SPONSORED</a>
			</div>
			<button class="create_project button blue" onclick="location.href=\''.md_get_durl().$prefix.'create_project=1\'">'.__('Create Project', 'memberdeck').'</button>
		</div><div class="col-lg-10"><div class="whitebox"><ul class="md-box-wrapper full-width cf"><li class="md-box full"><div class="author-'.$user_id.'" data-author="'.$user_id.'">';
	echo '<div class="dashboard-block"><a name="your_dreams"></a><h3>'.__('Your Dreams', 'memberdeck').': </h3>';
	echo '<ul>';
	$creator_args = array(
		'post_type' => 'ignition_product',
		'author' => $user_id,
		'posts_per_page' => -1,
		'post_status' => array('draft', 'pending', 'publish', 'closed')
	);
	$user_projects = apply_filters('id_creator_projects', get_posts(apply_filters('id_creator_args', $creator_args)));
	if (!empty($user_projects)) {
	...
	else echo '<li>No Dreams Built</li>';
	echo '</ul></div>';
	echo '<div class="dashboard-block"><a name="sponsored"></a><h3>'.__('Sponsored Dreams', 'memberdeck').': </h3>';
	echo '<ul>';
	//sponsored
	$misc = ' WHERE user_id = "'.$user_id.'"';$listed = array();$wp_ids = array();
	...
	if (!empty($orders)) {
		...
		$args = array('post_type' => 'ignition_product',/*'author' => $user_id,*/ 'post__in' =>$wp_ids, 'post_status' => array('publish', 'closed'), 'posts_per_page' => -1);
		$user_projects = apply_filters('id_creator_projects', get_posts(apply_filters('id_creator_args', $args)));
		$sp_query = new WP_Query( $args );
		if (!empty($user_projects)) {
			foreach ($user_projects as $post) {
				$post_id = $post->ID;setup_postdata($post);//$post->the_post();
				$project_id = get_post_meta($post_id, 'ign_project_id', true);
				if (!empty($project_id)) {
					$status = $post->post_status;
					if (strtoupper($status) !== 'TRASH') {
						...
						include IDC_PATH.'templates/_myProjectsSp.php';
					}
				}
			}
		}
	}
	else echo '<li>No Dreams Sponsored</li>';
...
function md_ide_notify_admin($user_id, $project_id, $post_id, $proj_args, $levels, $project_fund_type) {
	write_log( "admin notify start:".$user_id.'-'.$project_id.'-'.$post_id );
...
function md_ide_notify_creator($user_id, $project_id, $post_id, $proj_args, $levels, $project_fund_type) {
	write_log( "creator notify start: ".$user_id.'-'.$project_id.'-'.$post_id );
...
function idc_success_notification_options() {
	echo '<option name="remind_me_later">'.__('Remind Me Later', 'memberdeck').'</option>';
...
function idc_success_notification_text() {
	...
	$remind_me_later = stripslashes(get_option('remind_me_later'));
	$success_notification_admin = stripslashes(get_option('success_notification_admin'));
	echo '<div class="form-row remind_me_later email_text" style="display: none">';
	wp_editor((isset($remind_me_later) ? $remind_me_later : ''), "remind_me_later_text");
	echo '</div>';
