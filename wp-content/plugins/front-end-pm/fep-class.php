...
/******************************************MAIN DISPLAY BEGIN******************************************/
   ...
   function main_shortcode_output()
	...
	$parent_id = ( isset( $_POST['parent_id'] ) ) ? absint( $_POST['parent_id'] ): 0;
        ...
        $newMsg = "<p><strong>as".__("Create New Message", 'fep').":</strong></p>";
		...
		echo __("Upon submission you will be directed to your dashboard", 'fep').":<br/>";
...
function view_message()
    {
      ...
      $threadOut .= "<table><tr><td width='15%' ></td><td><strong style='display: inherit;color: #808080;'>".__("Subject", 'fep').": ".fep_output_filter($wholeThread[0]->message_title, true)."<strong></td></tr>";
      ...
      foreach ($wholeThread as $post)
      {
        ...
        $threadOut .= "<tr><td width='15%' align=\"center\"><a href='".fep_action_url()."between&with=".fep_get_userdata( $post->from_user, 'user_login', 'id' )."'>" .fep_get_userdata( $post->from_user, 'display_name', 'id' ). "</a><br/><small><a href='".fep_action_url()."viewmessage&id=$pID&order=$anti_order'></a></small>".get_avatar($post->from_user, 60)."<small style='color: navy;'><br/>".fep_format_date($post->send_date)."</small></td>";

        if ($post->parent_id == 0) //If it is the parent message
        {
          $threadOut .= "<td class='fep-pmtext' style='color: #808080;'><hr/>".fep_output_filter($post->message_contents)."";
		  ob_start();
      ...
      if ($post->status == 0 && $user_ID != $post->last_sender && ( $user_ID == $post->from_user || $user_ID == $post->to_user )) //Update only if the reader is not last sender
        ...
        {
          $threadOut .= "<td class='fep-pmtext' style='color: #808080;'>".fep_output_filter($post->message_contents)."";
		  ob_start();
