  ...
  class fep_announcement_class
  {
	...
	function user_settings_filter( $options )
	  ...
	  //var_dump($sliced_announcement);
        foreach ($sliced_announcement as $msg)
        {
		  ...
		  $msgsOut .= "<td><a href='".fep_action_url()."viewannouncement&id=$msg->id'>".fep_output_filter($msg->message_title, true)."</a><br/></td>";
		  if ( $this->is_seen($msg->id) )
		  $msgsOut .= "<td><a style='color: red;' href='".fep_action_url()."delannouncement&id=$msg->id&token=$token' onclick='return confirm(\"".__('Are you sure?', 'fep')."\");'>".__("Delete", 'fep')."</a></td>";
		  else
    ...
    function dispAnnouncement()
    {
	  ...
	  $this->make_seen( $id );
        ...
        echo "<strong>".__("Announcement", 'fep').":</strong>";
        echo "<table>";
		  echo "<tr class='fep-trodd0'><td class='fep-pmtext'  ><div class='col-md-2'></div><div class='col-md-6' style='color: #808080;'>".__("Global Announcement", 'fep').": ".fep_output_filter($announcement->message_title, true)."</div>";

		  
        echo "</td></tr>";

		  echo "<tr class='fep-trodd1'><td class='fep-pmtext'><strong>".__("Subject", 'fep').":</strong><div class='fep_annuncement_1 col-md-2'><div class='name_1' align='center'style='color: #808080;'> ".fep_get_userdata( $announcement->from_user, 'display_name', 'id' )."</div>".
			  "<strong>".__("Date", 'fep').":</strong><div class='avatar_float' align='center'>".get_avatar($user_ID, 55)."</div>";
		  if (current_user_can('manage_options')) {
			  echo "<strong>".__("Added by", 'fep').":</strong><div align='center' > <small>".fep_format_date($announcement->send_date)."</small></div></div>";
			  echo "<div class='fep_annuncement_1 col-md-6'>".fep_output_filter($announcement->message_content)."</div>";
			   }
