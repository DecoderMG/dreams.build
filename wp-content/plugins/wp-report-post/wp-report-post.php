...
?>
	...
	var Data = $(".fancybox-inner #wp-report-post-body-popup form").serialize();
    $.ajax({
        type: "POST",
        dataType: "json",
        ...
        success: function(data){
            ...
            if (data.error) {
                $.each(data.errors, function(e) {
	                console.log(e);
                    $(".fancybox-inner #wp-report-post-body-popup form #"+e).addClass("wp-report-post-error");
                });
            }
            if (data.success)  {
                $("#wp-report-post-form").slideUp(600, function() { $(".wp-report-post-sent").slideDown(600);  });
                 /*$.fancybox.close(); */
            }
      ...
      /*  $.post( "<?php echo admin_url( "admin-ajax.php" ); ?>", $("#wp-report-post-form").serialize(), function(data)
        {
	        console.log($('#wp-report-post-form textarea').val());
            console.log(data.last_query);
            console.log(data.last_error);
            if (data.error)
            {
