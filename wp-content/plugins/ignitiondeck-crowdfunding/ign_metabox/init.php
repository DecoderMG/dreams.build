...
//									'<div></div>';
				...
				case 'digitalfiles':
					...
					$digital_levels = get_post_meta($post->ID, 'ign_digital_levels', true);
						if($digital_levels>0) {
							echo '<table cellpadding="2" cellspacing="2">';
								echo '<thead>';
									echo '<tr>';
										echo '<th>'.__('Filename','ignitiondeck').'</th>';
										echo '<th>'.__('Title','ignitiondeck').'</th>';
										echo '<th>'.__('Description','ignitiondeck').'</th>';
										echo '<th>'.__('Actions','ignitiondeck').'</th>';
									echo '</tr>';
								echo '</thead>';
								echo '<tbody>';
								for($i=0;$i<$digital_levels;$i++){
									//$_SERVER['HTTP_POST'].'/wp-content/uploads/
									$filename = explode('/', get_post_meta($post->ID,'ign_digitals_'.$i.'_file',true));
									$filename = $filename[(count($filename)-1)];
									echo '<tr>';
										echo '<td><a class="btn btn-green" href="'.get_post_meta($post->ID,'ign_digitals_'.$i.'_file',true).'">'.$filename.'</a></td>';
										echo '<td>'.get_post_meta($post->ID,'ign_digitals_'.$i.'_name',true).'</td>';
										echo '<td>'.get_post_meta($post->ID,'ign_digitals_'.$i.'_description',true).'</td>';
										echo '<td><a data-index="'.$i.'" class="remove_digital_file" href="#">'.__('Delete','ignitiondeck').'</a></td>';
										echo '<input type="hidden" value="'.get_post_meta($post->ID,'ign_digitals_'.$i.'_file',true).'" name="ign_digitals_'.$i.'_file" class="digitalfile_'.$i.'" />';
										echo '<input type="hidden" value="'.get_post_meta($post->ID,'ign_digitals_'.$i.'_name',true).'" name="ign_digitals_'.$i.'_name" class="digitalfile_'.$i.'" />';
										echo '<input type="hidden" value="'.get_post_meta($post->ID,'ign_digitals_'.$i.'_description',true).'" name="ign_digitals_'.$i.'_description" class="digitalfile_'.$i.'" />';
									echo '</tr>';
								}
								echo '</tbody>';
								echo '</table>';
								echo '<input type="hidden" name="ign_digital_levels" class="ign_digital_levels" value="'.get_post_meta($post->ID, 'ign_digital_levels', true).'" />';
						} else {
							echo '<p>'.__('No digital files available at this moment.','ignitiondeck').'</p>';
						}
						echo '</div>';
				break;
	...
	function save($post_id, $post)  {
		...
		// Update digital files index
		$digital_levels = get_post_meta($post_id, 'ign_digital_levels', true);
		if($digital_levels>0) {
				// Remove old meta
				for($i=0;$i<$digital_levels;$i++) {
				...
				foreach($_POST as $post_key => $post_value) {
					if (preg_match("/ign_digitals_([0-9]+)_([a-z]+)/", $post_key, $update_key)) {
						$new_key = preg_replace('/ign_digitals_([0-9]+)_([a-z]+)/', 'ign_digitals_'.$new_index.'_$2', $post_key);
						add_post_meta( $post_id, $new_key, $post_value, false );
						$new_index++;
					}
				}
				update_post_meta($post_id, 'ign_digital_levels', (int)$_POST['ign_digital_levels']);
		}
	...
	?>
		...
		(function($){
			$(document).ready(function(){
				$(".remove_digital_file").on('click',function(e){
					e.preventDefault();
					var file_index = $(this).data('index'),
						ign_digital_levels = $(".ign_digital_levels"),
						current = (ign_digital_levels.val())*1;
					$(".digitalfile_"+file_index).remove();
					// Remove ui container
					$(this).parent().parent().remove();
					// Update levels
					ign_digital_levels.val(current-1);
