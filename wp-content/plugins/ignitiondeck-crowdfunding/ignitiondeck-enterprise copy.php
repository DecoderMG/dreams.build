...
function id_submissionForm($post_id = null) {
	global $wpdb;
	global $permalink_structure; global $vars;
	...
	write_log( "id_submissionForm start a bit later" );
	...
	if (!empty($post_id) && $post_id > 0) {
		...
		else if (!empty($vars['project_tags'])) {
			$project_tags = $vars['project_tags'];
		}	
		else {
			$project_tags = null;
		}
	...
	write_log( "id_submissionForm project type" );
		...
		if (is_user_logged_in()) {
			...
			if (isset($_POST['project_post_id'])) {
				...
				//$args['post_category'] = array($project_category);
				//$args['post_tag'] = $project_tags;*/
