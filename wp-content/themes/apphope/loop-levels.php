...
if (class_exists('Deck')) {
	...
	/*foreach ($levels as $key => $level) {
		if($level->meta_title =='Custom'||$level->meta_title =='No Incentive, just sponsoring'){
			//$level->meta_title = 'No Incentive, just sponsoring';
			$custom = $level;
			unset($levels[$key]);
		}
	}
	if(!empty($custom)) array_unshift($levels , $custom);*/
	foreach ($levels as $level) {
		...
		<?php if (empty($type) || $type == 'level-based') {
			if ($end_type == 'closed' && $days_left <= '0') { ?>
				<a class="level-binding btn">
			<?php
			} 
			else {
			?>
				<a class="level-binding btn" <?php echo (isset($level_invalid) && $level_invalid ? '' : 'href="'.apply_filters('id_level_'.$level->id.'_link', $url.'&level='.$level->id, $project_id).'"'); ?>>
		<?php 
			}
		} ?>
			<div class="level-group">
				<div class="ign-level-title">
					<span> <?php echo $level->meta_title ?></span>
						<?php global $currency;?>
					<div class="level-price" title="<?=$currency*$level->meta_price?> EUR">
