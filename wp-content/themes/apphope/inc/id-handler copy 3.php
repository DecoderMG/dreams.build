...
function check_vars($vars){
	...
	for ($i = 0; $i < $digital_levels; $i++) {
		if($_FILES['digital_file']['size'][$i] >= 1024*1024*100){
...
function id_submissionForm_action($post_id=null) {
	...
	if (isset($_POST['project_fesubmit']) || isset($_POST['project_fesave'])) {
		...
		for ($i = 0; $i < $digital_levels; $i++) {
			...
			if (isset($_FILES['digital_file']['name'][$i]) && $_FILES['digital_file']['size'][$i] > 0&& $_FILES['digital_file']['size'][$i] < 1024*1024*100) {
				$file = array(
		      'name'     => $_FILES['digital_file']['name'][$i],
		      'type'     => $_FILES['digital_file']['type'][$i],
		      'tmp_name' => $_FILES['digital_file']['tmp_name'][$i],
		      'error'    => $_FILES['digital_file']['error'][$i],
		      'size'     => $_FILES['digital_file']['size'][$i]
		    );
			...
			if (isset($post_id)) {
				...
				if (isset($project_id)) {
					...
					if(!$update && empty($saved_levels[0]['title']) && $project_levels===1){
						$saved_levels[0] = array('title'=>'No Incentive, just sponsoring','price'=>'','short'=>'','long'=>'','limit'=>0);
					}
					...
					if(!$update){
						...
						if($last_one>1){
							...
							update_post_meta($post_id, 'ign_product_level_'.($last_one).'_price', '');
							update_post_meta($post_id, 'ign_product_level_'.($last_one).'_short_desc', '');
							update_post_meta($post_id, 'ign_product_level_'.($last_one).'_desc', '');
